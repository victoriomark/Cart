...
function HomeProduct() {
   ...
   useEffect(() => {
      axios
         .get(`https://fakestoreapi.com/products?limit=${NumberOfItem}`)
         .then((Data) => {
   ...
   return (
      <>
         <div className='row gap-3 justify-content-center mt-5'>
            {AllItems.map(({ title, price, image, id }) => {
               return (
                  <div
                     key={id}
                     className='card bg-black text-light   col-lg-2 p-2'
                  >
                     <img
                        style={{
                           objectFit: "cover",
         ...
         <div className='container flex-column text-info mt-3 p-2  d-flex justify-content-center align-items-center   '>
           <div className="gap-5 d-flex ">
           <button onClick={HanldePrevItem} className='btn btn-dark text-info'>
               <i className='fa-solid fa-arrow-left'></i>
            </button>
            <button onClick={HanldeMoreItem} className='btn btn-dark text-info'>
               <i className='fa-solid fa-arrow-right'></i>
            </button>
           </div>
           <h5>Load More Product</h5>
         </div>
      </>
   );
}
